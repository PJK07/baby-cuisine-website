...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // ⚡ Bolt: Memoize cart item count to prevent O(N) array reduction recalculation on every consumer render
  const totalItems = useMemo(() =>
    items.reduce((total, item) => total + item.quantity, 0), [items]);

  // ⚡ Bolt: Memoize cart price calculation to prevent O(N) array reduction recalculation on every consumer render
