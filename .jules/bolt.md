...
## 2024-05-26 - Memoize derived React Context properties
**Learning:** Exposing functions in React Context that calculate derived state (such as array `.reduce` calls for `getTotalItems` and `getTotalPrice` in `CartContext.tsx`) causes O(N) recalculations on every render of every consumer component, degrading frontend performance.
**Action:** Replace derived state calculation functions in Context with `useMemo` properties (e.g. `totalItems` and `totalPrice`) to maintain O(1) render time complexity for all consuming components.
