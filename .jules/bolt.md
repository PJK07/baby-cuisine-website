...
## 2026-06-14 - Memoize derived state in React Context
**Learning:** Exposing functions that calculate derived state (e.g. `getTotalItems` with array reduction) in React Context causes O(N) recalculations on every single render of consuming components, blocking the main thread when state is large.
**Action:** Extract these expensive operations using `useMemo` and expose memoized properties instead of computation functions to maintain O(1) render time complexity for context consumers.
