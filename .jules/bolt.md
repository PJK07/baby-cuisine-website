...
## 2024-05-26 - Memoize Context derived state properties instead of exposing computation functions
**Learning:** In React contexts like `CartContext.tsx`, exposing functions that calculate derived state from array reductions (like `getTotalItems` or `getTotalPrice`) forces O(N) re-calculations on the main thread every time a consumer component renders and calls them.
**Action:** Instead of exposing functions, use `useMemo` to memoize the derived state properties (`totalItems`, `totalPrice`) internally within the provider and expose the memoized properties directly to consumers, reducing consumer reads to O(1) time complexity.
