...
## 2024-05-29 - Native Image Lazy Loading for Long Product Lists
**Learning:** In list views with many images (like the Shop menu categories), rendering all images eagerly on initial mount downloads large amounts of unnecessary image data below the fold, causing network congestion, higher data usage, and a slower perceived page load time.
**Action:** Always add `loading="lazy"` to `<img>` tags inside repeating map components or long lists so the browser defers loading them until they are near the viewport.
