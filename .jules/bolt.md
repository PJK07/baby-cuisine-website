...
**Learning:** Exposing state calculation functions (like `getTotalPrice` which does a `.reduce()`) from a React Context means every consumer re-running those functions causes O(N) recalculations per render, blocking the main thread if N is large.
**Action:** Extract derived state calculations into `useMemo` properties inside the context provider and expose those properties instead of the functions. This ensures the calculation only happens once when the dependency array changes.
