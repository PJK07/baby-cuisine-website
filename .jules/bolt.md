...
## 2024-05-26 - Memoizing Derived State in React Context
**Learning:** Returning getter functions (like `getTotalItems()`) from a React Context provider forces consumers to run O(N) `Array.reduce` calculations on every render loop, creating unnecessary overhead across the entire app.
**Action:** Always avoid exposing functions that calculate derived state in Context values. Instead, use `useMemo` within the provider to calculate the derived properties (e.g. `totalItems`) and expose those memoized properties directly to consumers.
