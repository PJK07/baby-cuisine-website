...
**Learning:** When managing React Context, exposing functions that calculate derived state (e.g., array reductions like calculating total items or price from a cart) forces an O(N) recalculation on every render of every consumer component.
**Action:** Use `useMemo` to expose memoized properties for derived state instead of getter functions, preventing unnecessary recalculations and maintaining O(1) consumer render performance.
