...
## 2024-10-25 - Context Provider Performance Anti-pattern
**Learning:** Exposed functions calculating derived state (like array reduction for cart totals) in React Context causes O(N) recalculations on consumer renders. Returning `getTotalItems()` meant `reduce()` was executed multiple times unnecessarily across components on every re-render.
**Action:** Use `useMemo` to expose memoized properties for derived state instead of exposing the calculation functions themselves.
